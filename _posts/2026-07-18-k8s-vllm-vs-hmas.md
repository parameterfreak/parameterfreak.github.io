...
H-MAS는 현재 파일럿 파트너와 함께 다양한 실 환경에서 검증을 진행하고 있습니다. GPU 서버를 보유하고 있고 AI 모델 서빙에 관심이 있는 조직이라면, 이 글의 시나리오를 여러분의 클러스터에서 직접 재현해 보실 수 있도록 온보딩을 지원합니다. 파일럿 기간 동안 라이센스와 기술지원은 무상으로 제공되며, 파트너 피드백은 개발 우선순위에 반영됩니다. 자세한 조건은 [파일럿 프로그램 안내](/hmas/pilot/)를 참고해 주세요.
