...
| GPU 활용률 관리 | 모니터링 도구 별도 구축 필요 | 클러스터별 리소스 현황 + 실시간 GPU·TPS·지연 메트릭 차트 *(v0.7 제공)* |
| 로그 확인 | 서버별 접속, kubectl logs | 웹에서 배포별 Pod 로그·이벤트 조회 + 추론 요청 로그 뷰어 *(v0.7 제공)* |
...
H-MAS에서 AI 모델을 배포하는 과정입니다. 모델 선택 → 배치 정책 → 런타임 → 리소스 설정 → 배포 실행까지, 컨트롤 플레인이 최적 클러스터를 자동으로 선택합니다.

<p align="center">
  <img src="/images/portfolio/hmas/pitch/deploy-workflow.png" alt="H-MAS 배포 워크플로우" width="900"/>
  <br/>
  <em>배포 워크플로우 — 모델 크기에 따라 HP/GP 클러스터로 자동 분기, 토폴로지 기반 최적 배치</em>
</p>
...
- vLLM, Ollama, TGI, llama.cpp, TEI 등 5종 런타임 지원 (4종 추가 예정)
...
- GPU 사용률, 메모리, TPS, 응답 지연시간(p95/p99) 통합 차트 *(v0.7 제공)*
- 통합 추론 API 프록시(OpenAI 호환) 및 추론 요청 로그 뷰어 *(v0.7 제공)*
- 추론 API Key 인증 및 Rate Limiting(RPM/TPM) *(v0.7 제공)*
...
<p align="center">
  <img src="/images/portfolio/hmas/pitch/architecture.png" alt="H-MAS 아키텍처" width="800"/>
  <br/>
...
> 아래 수치는 하드웨어 스펙 및 관련 연구 기반의 **예상 효과**이며, v0.9(Technical Preview)에서 실측 벤치마크를 통해 검증할 예정입니다.

### HP 클러스터 — 대형 모델 (70B+)

기본 Kubernetes 스케줄러 대비 기대 개선 수치입니다.

```
처리량 (Throughput)     ████████████████░░░░  1.5x ~ 1.7x 향상 (예상)
응답 지연시간 (Latency)  ████████████░░░░░░░░  29% ~ 42% 감소 (예상)
GPU 간 통신 효율         ████████████████████  14x ~ 28x 향상 (하드웨어 스펙)
```
...
```
처리량 (Throughput)     ████████████░░░░░░░░  1.26x ~ 1.55x 향상 (예상)
GPU 활용률              ████████████████████  25% → 90%+ (목표)
비용 효율               ████████████████░░░░  2x ~ 3x 개선 (예상)
```
...
| 런타임 | 특징 | 주요 용도 | 지원 상태 |
|-------|------|----------|----------|
| **vLLM** | PagedAttention, Continuous Batching | 고성능 LLM 서빙 (추천) | v0.4 지원 |
| **Ollama** | 간편한 설정, 빠른 시작 | 로컬 LLM 실행, 프로토타이핑 | v0.4 지원 |
| **TGI** | HuggingFace 생태계 통합 | HuggingFace 모델 프로덕션 서빙 | v0.4 지원 |
| **llama.cpp** | CPU/GPU 효율적 추론, GGUF | 경량 환경, CPU 추론 | v0.4 지원 |
| **TEI** | HuggingFace Text Embeddings 최적화 | 임베딩 모델 서빙 | v0.4 지원 |
| **Triton** | 멀티 프레임워크, 앙상블 모델 | 복합 추론 파이프라인 | 예정 |
| **SGLang** | RadixAttention, 구조화된 생성 | 구조화된 출력 최적화 | 예정 |
| **TensorRT-LLM** | NVIDIA GPU 극한 최적화 | 최고 성능이 필요한 경우 | 예정 |
| **LMDeploy** | TurboMind 엔진, 다양한 모델 | 범용 고성능 서빙 | 예정 |
...
<p align="center">
  <img src="/images/portfolio/hmas/pitch/cluster-classification.png" alt="클러스터 관리 화면" width="800"/>
  <br/>
  <em>클러스터 관리 — GPU 타입별 분류, 클러스터별 GPU 사용량과 스케줄러 정보 확인</em>
</p>
...
<p align="center">
  <img src="/images/portfolio/hmas/pitch/gpu-topology.png" alt="GPU 토폴로지 시각화" width="800"/>
  <br/>
  <em>GPU 토폴로지 — 노드별 NUMA 구조와 GPU 배치, NVLink/PCIe 인터커넥트 연결 시각화</em>
</p>
...
- 배포별 Pod 로그 조회 및 K8s 이벤트 확인
- 감사 로그 (플랫폼 활동 이력)
- GPU 사용률, 메모리, TPS, 지연시간(p95/p99) 실시간 차트 *(v0.7 제공)*
- 추론 요청 로그 뷰어 — 요청/응답 상세, 토큰 사용량, 지연시간 *(v0.7 제공)*
...
| **2026년 7월** | **v0.7 (FP)** | Feature Preview — Control Plane 고도화(통합 추론 API·모니터링·인증/인가), GP 서빙 엔진 |
| **2026년 9월** | **v0.9 (TP)** | Technical Preview — HP 서빙 엔진(토폴로지 인식), 모델 로딩 가속, 통합 검증 |
...
| 기능 | v0.4 (DP) | v0.7 (FP) | v0.9 (TP) | v1.0 (GA) |
|------|:---------:|:---------:|:---------:|:---------:|
| 멀티 클러스터 관리 (Push) | v | v | v | v |
| 멀티 클러스터 관리 (Pull, 에이전트) | — | — | v | v |
| 웹 콘솔 기반 모델 배포 | v | v | v | v |
| 통합 대시보드 | v | v | v | v |
| 자동 장애 복구 | v | v | v | v |
| 로그 조회/이벤트/감사 로그 | v | v | v | v |
| 통합 추론 API 프록시 (OpenAI 호환) | — | v | v | v |
| 실시간 메트릭 모니터링 (GPU, TPS, 지연) | — | v | v | v |
| 추론 요청 로깅 / Rate Limiting | — | v | v | v |
| GP 스케줄링 (QoS, GPU 공유) | — | v | v | v |
| HP 스케줄링 (토폴로지 인식) | — | — | v | v |
| 모델 로딩 가속 (분산 캐시) | — | — | v | v |
| 모델 저장소 (Built-in Registry) | v | v | v | v |
| 모델 레지스트리 (외부 연동) | — | — | v | v |
| 기본 인증 (JWT) | v | v | v | v |
| 인증/인가 고도화 (RBAC, API Key) | — | v | v | v |
| 멀티테넌시 (Organization/Group) | — | v | v | v |
| 외부 인증 연동 (OAuth/OIDC) | — | — | v | v |
| 프로덕션 안정화 | — | — | — | v |
| 운영 도구 (알림, 헬스체크 자동화) | — | — | — | v |
...
| 버전 | 날짜 | 요약 | 릴리즈 노트 |
|------|------|------|------------|
| v0.7 (FP) | 2026-07 | Feature Preview — 통합 추론 API 프록시(OpenAI 호환), 실시간 모니터링, 인증/인가 고도화(RBAC·API Key), 멀티테넌시, GP 서빙 엔진 | — |
| v0.4.0 (DP) | 2026-04-05 | Developer Preview — 멀티 클러스터 관리, 웹 콘솔, JWT 인증, 원커맨드 설치 | [릴리즈 노트](/posts/2026/04/h-mas-v0.4.0-release/) |
...
**Q. 현재(v0.7)에서 사용 가능한 기능이 제한적이지 않나요?**
멀티 클러스터 관리, 웹 콘솔 배포, 대시보드, 자동 장애 복구에 더해, v0.7에서는 통합 추론 API 프록시(OpenAI 호환), 실시간 모니터링(TPS·지연·GPU), 인증/인가 고도화(RBAC·API Key), 멀티테넌시, GP 서빙 엔진까지 사용할 수 있습니다. GPU 토폴로지 인식 스케줄링(HP 서빙)은 v0.9에서 제공될 예정이며, 파일럿 파트너에게는 새 기능이 지속적으로 업데이트됩니다.
...
**Q. 보안/인증은 지원하나요?**
v0.7에서는 JWT 기반 로그인 인증에 더해 RBAC(역할 기반 접근 제어), 사용자 관리, 추론 API Key 인증, Rate Limiting, 멀티테넌시(Organization/Group)를 제공합니다. OAuth/OIDC 외부 인증 연동은 v0.9에서 추가될 예정입니다.
...
| **현재 버전** | Feature Preview (v0.7) — 2026년 7월 출시 |
